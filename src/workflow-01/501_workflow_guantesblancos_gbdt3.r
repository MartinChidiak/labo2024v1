...
envg$EXPENV <- list()
envg$EXPENV$exp_dir <- "~/buckets/b1/exp9/"
dir.create(envg$EXPENV$exp_dir, showWarnings = FALSE)
envg$EXPENV$wf_dir <- "~/buckets/b1/flow9/"
dir.create(envg$EXPENV$wf_dir, showWarnings = FALSE)
envg$EXPENV$wf_dir_local <- "~/flow9/"
...
#   entreno en solo tres meses   ( mas guantes blancos no se puede ) PASO A 10 MESES
...
HT_tuning_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$lgb_param <- list(
    ...
    num_leaves = c( 100L, 1000L,  "integer" ),
